...
export class ArchivosComponent implements OnInit {
  ...
  renderMsg() {
    for (let i in this.typesState) {
      if (this.typesState[i].obligatorio === 'S') {
        if (!this.typesState[i].download.file.length) {
          this.sendMsg.emit({
            descripcionStep: 'Archivos',
            id: String(this.typesState[i].id+this.typesState[i].descripcion),
            mensaje: `${this.typesState[i].descripcion}` + ', archivo requerido antes de enviar a control',
            codigoStep: 11
          });
        } else {
          this.removeMsg.emit({
